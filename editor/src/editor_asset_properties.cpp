...
bool UpdateShaderPropertyGrid(wxPropertyGrid* pg, wxTreeListCtrl* treeList, wxTreeListEvent& event, LE3AssetManager& assets)
{
    ...
    pg->Append(new wxFileProperty(wxT("Vertex Shader Path"), wxPG_LABEL, assets.m_shaders[name.ToStdString()].m_vertexShaderPath));
    pg->Append(new wxFileProperty(wxT("Fragment Shader Path"), wxPG_LABEL, assets.m_shaders[name.ToStdString()].m_fragmentShaderPath));
